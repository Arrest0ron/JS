...
export class EditTrackPage {
    ...
    getData() {
        ...
        else ajax.get(trackUrls.getTrackById(this.id), (data) => {
            console.log("Creating data for track ", this.id);
   ...
   renderData(item) {
    ...
    document.getElementById("placeholder-release-year").innerHTML =
        `<strong>Год выпуска:</strong> ${item.releaseYear || "2023"}`;
}
...
render() {
    ...
    document.getElementById("release_year_inp").addEventListener("input", (e) => {
        document.getElementById("placeholder-release-year").innerHTML = `<strong>Год выпуска:</strong> ${e.target.value || "2023"}`;
    });
}
   ...
   saveData() {
    ...
    if (this.id === -1) {
        // Create new track
        ajax.get(trackUrls.getTracks(), (tracks) => {
            const newId = tracks[tracks.length - 1]?.id + 1 || 1;
            data.id = newId;

            ajax.post(trackUrls.createTrack(), data, () => {
                this.id = newId;
                this.render();
                    const mainPage = new MainPage(document.getElementById('root'))
                    mainPage.render()
        ...
        ajax.patch(trackUrls.updateTrackById(this.id), data, () => {
            this.render();
                const mainPage = new MainPage(document.getElementById('root'))
                mainPage.render()
