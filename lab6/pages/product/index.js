...
export class TrackPage {
    ...
    async render() {
        console.log("Loading track details for ID:", this.id);

        try {
            const data = await ajax.get(trackUrls.getTrackById(this.id));

            if (!data) {
                console.error("No data received for track ID:", this.id);
                this.parent.innerHTML = '<p>Ошибка: данные о треке не найдены.</p>';
                return;
            }
            ...
            if (logo) {
                logo.addEventListener("click", this.clickBack);
            }

        } catch (err) {
            console.error("Failed to load track details", err);
            this.parent.innerHTML = `<p>Произошла ошибка при загрузке данных: ${err.message}</p>`;
        }
    }
}